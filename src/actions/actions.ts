...
export const getMailMasseges = (id:number)=> {
    return (axios.get(`https://hiring.reachinbox.xyz/api/v1/onebox/messages/:thread_id`,{
        headers: {
            'authorization': `Bearer ${token}`
        }
    }).then ((res) => res.data.data)
...
export const postMailMasseges = (id:number,messages:any)=> {
    return axios.post(`https://hiring.reachinbox.xyz/api/v1/onebox/reply/:thread_id`, messages ,{
        headers: {
            'Authorization': `Bearer ${token}`
        }
    })
